...
(({ behaviors }) => {
  behaviors.storytheme_storybook_m_card = {
    defaultEntry: () => {
      return {
        className: 'm-card',
        processingName: 'storybook-m-card',
      };
    },
    customEntry: () => {
      ...
      // construction:
      //     (({ behaviors }) => {
      //       behaviors.storytheme_storybook_m_card.customEntry = () => {
      //         return [
      //           {
      //             ...your configuration,
      //           },
    ...
    entries: () => {
      let entries = [behaviors.storytheme_storybook_m_card.defaultEntry()];
      if (behaviors.storytheme_storybook_m_card.customEntry()) {
        entries.push(...behaviors.storytheme_storybook_m_card.customEntry());
      }
      return entries;
    },
    attach: (context) => {
      behaviors.storytheme_storybook_m_card.entries().forEach((entry) => {
        once(
