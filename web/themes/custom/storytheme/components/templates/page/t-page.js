 ...
 */
(({ behaviors }) => {
  behaviors.storytheme_storybook_t_page = {
    defaultEntry: () => {
      return {
        className: 't-page',
        processingName: 'storybook-t-page',
      };
    },
    customEntry: () => {
      ...
      // construction:
      //     (({ behaviors }) => {
      //       behaviors.storytheme_storybook_t_page.customEntry = () => {
      //         return [
      //           {
      //             ...your configuration,
      //           },
    ...
    entries: () => {
      let entries = [behaviors.storytheme_storybook_t_page.defaultEntry()];
      if (behaviors.storytheme_storybook_t_page.customEntry()) {
        entries.push(...behaviors.storytheme_storybook_t_page.customEntry());
      }
      return entries;
    },
    attach: (context) => {
      behaviors.storytheme_storybook_t_page.entries().forEach((entry) => {
        once(
          entry.processingName,
          `.${entry.className}`,
          context,
        ).forEach((el) => {
          behaviors.storytheme_storybook_t_page.handler({ el, entry });
        });
      });
    },
    handler: (obj) => {
